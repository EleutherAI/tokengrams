...
use crate::sushi_sort::sais_table;
...
impl MemmapIndex {
    ...
    #[staticmethod]
    pub fn build_sushi(text_path: String, table_path: String, verbose: bool) -> PyResult<Self> {
        // Memory map the text as read-only
        let text_mmap = MmapSlice::new(&File::open(&text_path)?)?;
        ...
        sais_table(text_mmap.as_slice(), table_mmap.as_slice_mut());
        if verbose {
            println!("Time elapsed: {:?}", start.elapsed());
        }
