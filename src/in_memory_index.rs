...
pub struct InMemoryIndex<T: Token> {
    table: SuffixTable<Box<T>, Box<u64>>,
    cache: KneserNeyCache,
}

impl<T: Token> InMemoryIndex<T> {
    pub fn new_py(_py: Python, tokens: Vec<u16>, verbose: bool) -> Self {
        InMemoryIndex {
            table: SuffixTable::<Box<T>, Box<u64>>::new(tokens, verbose),
...
impl<T: Token> Sample<T> for InMemoryIndex<T> {
...
macro_rules! create_in_memory_interface {
    ($name: ident, $type: ident) => {
        #[pyclass]
        pub struct $name {
            table: SuffixTable<Vec<$type>, Vec<u64>>,
            cache: KneserNeyCache,
        }
        ...
        impl $name {
            ...
            #[staticmethod]
            pub fn from_pretrained(path: String) -> PyResult<Self> {
                let table: SuffixTable<Vec<$type>, Vec<u64>> = deserialize(&std::fs::read(path)?).unwrap();
                Ok($name {
                    table,
                    cache: KneserNeyCache::default(),
                })
