...
fn sample_unsmoothed_exists() {
    ...
    let index = InMemoryIndexU16::new(s, false);
    let seqs = index
...
fn sample_unsmoothed_empty_query_exists() {
    let s = "aaa".encode_utf16().collect::<Vec<_>>();
    let index = InMemoryIndexU16::new(s.clone(), false);
    let seqs = index
...
fn sample_smoothed_exists() {
    ...
    let mut index = InMemoryIndexU16::new(s.clone(), false);
...
fn smoothed_probs_exists() {
    ...
    let bigram_counts = sa.count_next(&query, Some(vocab as usize));
    ...
    let mut index = InMemoryIndexU16::new(s.clone(), false);
    let smoothed_probs = index.get_smoothed_probs(query.clone(), Some(vocab as usize));
...
fn smoothed_probs_empty_query_exists() {
    let s = "aaa".encode_utf16().collect::<Vec<_>>();
    let mut index = InMemoryIndexU16::new(s, false);
    let vocab = utf16!("a")[0] as usize + 1;

    let probs = index.get_smoothed_probs(Vec::new(), Some(vocab));
